...
def home(request):
    context = {
        ...
        "latest_jobs": Opportunity.objects.filter(
            status="active", category__slug="jobs"
        ).select_related("organization", "category").order_by("-created_at")[:3],
        "latest_scholarships": Opportunity.objects.filter(
            status="active", category__slug="scholarships"
        ).select_related("organization", "category").order_by("-created_at")[:3],
        ...
        "stats": {
            ...
            "active_jobs": Opportunity.objects.filter(status="active", category__slug="jobs").count(),
            "scholarships": Opportunity.objects.filter(status="active", category__slug="scholarships").count(),
